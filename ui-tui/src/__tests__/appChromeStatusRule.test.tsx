...
describe('StatusRule idle-since read-out', () => {
  ...
  it('shows the gateway duration unchanged regardless of wall-clock timestamps', () => {
    const element = StatusRule({
      ...baseProps,
      lastTurnEndedAt: Date.now() - 600_000,
      sessionStartedAt: Date.now() - 900_000,
      usage: { ...baseProps.usage, last_turn_seconds: 42 }
    })

    expect(textContent(element)).toContain('✓ 42s')
