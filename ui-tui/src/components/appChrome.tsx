...
import { fmtMetricSeconds, statusPerformanceLabels } from '../lib/statusPerformance.js'
...
export function StatusRule({
  ...
  // FaceTicker owns the live timer. Once complete, render the gateway's frozen
  // wall time; it must not turn into an ever-growing idle clock.
  const lastTurnSeconds = Math.max(0, usage.last_turn_seconds || 0)
  const lastTurnText = lastTurnSeconds > 0 ? `✓ ${fmtMetricSeconds(lastTurnSeconds)}` : ''
  const showLastTurn = segs.duration && !busy && !!lastTurnText && fits(SEP + stringWidth(lastTurnText))

  const performanceLabels = cols >= 96 ? statusPerformanceLabels(usage) : []
  const visiblePerformanceLabels = performanceLabels.filter(label => fits(SEP + stringWidth(label)))
