...
def _is_agentrouter_endpoint(base_url: str | None) -> bool:
    """Return True for AgentRouter's Anthropic-compatible endpoint."""
    return base_url_host_matches(base_url or "", "agentrouter.org")
...
# Custom Anthropic relays may require either x-api-key or Bearer auth.
# Resolve by base URL here so every client construction path stays aligned.
def _configured_anthropic_auth(base_url: str | None) -> str:
    ...
    try:
        ...
        for entry in get_compatible_custom_providers():
            candidate = _normalize_base_url_text(entry.get("base_url")).rstrip("/").lower()
            if candidate != normalized:
                continue
            mode = str(entry.get("anthropic_auth") or "auto").strip().lower()
            if mode in {"auto", "x_api_key", "bearer"}:
                return mode
    except Exception:
        logger.debug("Could not resolve configured Anthropic relay auth", exc_info=True)
    return "auto"


def _configured_anthropic_extra_headers(base_url: str | None) -> dict[str, str]:
    """Resolve per-provider headers for every Anthropic client path.

    OpenAI-wire clients already receive ``providers.<name>.extra_headers``
    in ``agent_init``. Anthropic primary, fallback, auxiliary, model-switch,
    and dashboard-probe clients all converge here, so resolving by base URL
    keeps those paths consistent. Header values may contain credentials and
    ...
    try:
        ...
        return get_custom_provider_extra_headers(normalized)
    except Exception:
        logger.debug("Could not resolve configured Anthropic relay headers", exc_info=True)
        return {}
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    explicit_auth = str(anthropic_auth or "").strip().lower()
    configured_auth = (
        explicit_auth if explicit_auth in {"bearer", "x_api_key"}
        else _configured_anthropic_auth(normalized_base_url)
    )

    if configured_auth == "bearer":
        kwargs["auth_token"] = api_key
        if common_betas:
            kwargs["default_headers"] = {"anthropic-beta": ",".join(common_betas)}
    elif configured_auth == "x_api_key":
    ...
    if _is_agentrouter_endpoint(base_url):
        # AgentRouter rejects generic SDK fingerprints at its WAF boundary
        # (often as HTTP 405) even when authentication is valid. Its
        # Anthropic-compatible relay currently accepts this CLI marker.
        headers = dict(kwargs.get("default_headers") or {})
        headers.setdefault("User-Agent", "claude-cli/1.0.0 (external, cli)")
        kwargs["default_headers"] = headers

    # Per-provider headers are the most specific configuration layer. Apply
    # them after SDK/provider defaults so custom relays can override a default
    # User-Agent or add WAF/routing headers. Resolving here also covers the
    # follow-up request after a tool result.
    configured_headers = _configured_anthropic_extra_headers(normalized_base_url)
