    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Session-scoped provider latency used by the TUI performance
                # status line. ``api_duration`` surrounds the real logical
                # provider call (including Hermes retries), so it is more
                # honest than deriving LLM time from turn wall time minus tool
                # durations. Keep the counter beside the existing session
                # token counters so model switches see one monotonic value.
                agent.session_llm_seconds = (
                    float(getattr(agent, "session_llm_seconds", 0.0) or 0.0)
                    + max(0.0, api_duration)
                )
                ...
                if response_invalid:
                    ...
                    if _resp_error_code == 524:
                        ...
                        _failure_hint = (
                            f"incomplete or malformed response stream "
                            f"after {api_duration:.0f}s"
                        )
                    else:
