...
#
# Why ``[`` is a separator: context-window variant suffixes are written
# in brackets (``claude-opus-5[1m]``, ``MiniMax-M3[1m]``).  Those are the
# same reasoning model with a wider window, so they must inherit the base
# slug's floor — a 1M-window variant is if anything *slower* to first
# token, so falling through to the 180s default is the worst outcome.
#
...
def get_reasoning_stale_timeout_floor(model: object) -> Optional[float]:
    ...
    600.0
    >>> get_reasoning_stale_timeout_floor("claude-opus-5[1m]")
    240.0
