...
def _persist_model_switch(result) -> None:
    ...
    # Keep the persisted wire protocol in lock-step with the provider/model.
    # Leaving the previous provider's api_mode behind makes a later desktop
    # restart send OpenAI-compatible GPT models through Anthropic Messages (or
    # vice versa), even though the in-memory switch used the correct client.
...
def _begin_turn_performance(agent, started_at: float | None = None) -> None:
    """Start one live turn's user-visible latency clock."""
    agent._perf_turn_started_monotonic = (
        float(started_at) if started_at is not None else time.monotonic()
    )
    agent._perf_first_token_monotonic = None


def _record_first_token_performance(agent, now: float | None = None) -> None:
    """Record end-to-end time to the first visible assistant token once."""
    if getattr(agent, "_perf_first_token_monotonic", None) is not None:
        return
    started_at = getattr(agent, "_perf_turn_started_monotonic", None)
    if started_at is None:
        return
    observed_at = float(now) if now is not None else time.monotonic()
    latency = max(0.0, observed_at - float(started_at))
    agent._perf_first_token_monotonic = observed_at
    agent.session_ttft_total_seconds = (
        float(getattr(agent, "session_ttft_total_seconds", 0.0) or 0.0)
        + latency
    )
    agent.session_ttft_count = int(
        getattr(agent, "session_ttft_count", 0) or 0
    ) + 1


def _finish_turn_performance(agent, now: float | None = None) -> None:
    """Freeze the last response duration; idle time must never change it."""
    started_at = getattr(agent, "_perf_turn_started_monotonic", None)
    if started_at is None:
        return
    ended_at = float(now) if now is not None else time.monotonic()
    agent.session_last_turn_seconds = max(0.0, ended_at - float(started_at))
    agent.session_completed_turns = int(
        getattr(agent, "session_completed_turns", 0) or 0
    ) + 1
    agent._perf_turn_started_monotonic = None
...
def _get_usage(agent) -> dict:
    ...
    def g(key, fallback=None):
        primary = _numeric(getattr(agent, key, 0))
        if primary:
            return primary
        if fallback:
            return _numeric(getattr(agent, fallback, 0))
        return 0
    usage = {
        ...
        "turns": g("session_completed_turns"),
        "tool_calls": g("session_tool_calls"),
        "llm_seconds": float(getattr(agent, "session_llm_seconds", 0.0) or 0.0),
        "tool_seconds": float(getattr(agent, "session_tool_seconds", 0.0) or 0.0),
        "last_turn_seconds": float(
            getattr(agent, "session_last_turn_seconds", 0.0) or 0.0
        ),
    }
    ttft_count = int(getattr(agent, "session_ttft_count", 0) or 0)
    if ttft_count > 0:
        usage["ttft_avg_seconds"] = (
            float(getattr(agent, "session_ttft_total_seconds", 0.0) or 0.0)
            / ttft_count
        )
    if usage["llm_seconds"] > 0 and usage["output"] > 0:
        usage["tokens_per_second"] = usage["output"] / usage["llm_seconds"]
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    ...
    if session is not None:
        agent = session.get("agent")
        if agent is not None:
            agent.session_tool_calls = int(
                getattr(agent, "session_tool_calls", 0) or 0
            ) + 1
            if duration_s is not None:
                agent.session_tool_seconds = (
                    float(getattr(agent, "session_tool_seconds", 0.0) or 0.0)
                    + max(0.0, duration_s)
                )
    try:
    ...
    *,
    ...
    def run():
        ...
        try:
            ...
            agent = session["agent"]
            _begin_turn_performance(agent, _turn_started_monotonic)
            ...
            def _stream(delta):
                if isinstance(delta, str) and delta:
                    _record_first_token_performance(agent)
                with session["history_lock"]:
            ...
            _finish_turn_performance(agent)
