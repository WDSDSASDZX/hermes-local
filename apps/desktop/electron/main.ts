...
function findPythonForRoot(root) {
  ...
  const candidates = sourcePythonCandidates(root, HERMES_HOME, {
    explicitVenv: process.env.HERMES_LOCAL_VENV,
    platform: process.platform
  })

  const pythonPath = [root, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)

  return findUsableSourcePython(candidates, findSystemPython(), candidate => {
    return (
      fileExists(candidate) &&
      canImportHermesCli(candidate, {
...
ipcMain.handle('hermes:window:control', (event, action) => {
  ...
  if (action === 'minimize') {
    ...
    return { ok: true, isMaximized: target.isMaximized() }
  }
  ...
  if (action === 'toggle-maximize') {
    ...
    sendWindowStateChanged(undefined, target)
