...
import { type FallbackEntry, normalizeFallbackEntries } from '@/app/settings/fallback-models'
...
import { contributedRoutes } from '../routes'
...
// Stable empty arrays keep unrelated store ticks from re-rendering inactive panels.
const EMPTY_SESSIONS: readonly never[] = []
const EMPTY_PINNED: readonly string[] = []
const EMPTY_SUBAGENTS: readonly never[] = []
...
export function CommandCenterView({
  ...
  const sessions = useStoreSelector($sessions, s =>
    section === 'sessions' || section === 'overview' ? s : EMPTY_SESSIONS
  )
  ...
  const subagents = useMemo(
    () => (section === 'overview' ? allSubagents(subagentsBySession) : EMPTY_SUBAGENTS),
    [section, subagentsBySession]
  )
  ...
  const kanbanAvailable = useMemo(() => contributedRoutes().some(route => route.path === '/kanban'), [])
  ...
  const refreshOverview = useCallback(async () => {
    ...
    const [statusResult, fallbackResult] = await Promise.allSettled([
      getStatus(),
      requestGateway<{ value?: unknown }>('config.get', { key: 'fallback_providers' })
    ])
    ...
    if (fallbackResult.status === 'fulfilled') {
      setFallbacks(normalizeFallbackEntries(fallbackResult.value?.value).filter(entry => entry.provider && entry.model))
    } else {
      const message =
        fallbackResult.reason instanceof Error ? fallbackResult.reason.message : String(fallbackResult.reason)

      if (!/unknown config key:\s*fallback_providers/i.test(message)) {
        errors.push(message)
      }
    }
