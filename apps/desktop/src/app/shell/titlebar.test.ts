...
describe('shouldUseCustomWindowControls', () => {
  it('fills the WSLg frameless gap when no native overlay exists', () => {
    expect(
      shouldUseCustomWindowControls({ isFullscreen: false, nativeOverlayWidth: 0, windowButtonPosition: null }, true)
    ).toBe(true)
  })

  it('defers to native controls and hides in fullscreen or browsers', () => {
    expect(
      shouldUseCustomWindowControls({ isFullscreen: false, nativeOverlayWidth: 138, windowButtonPosition: null }, true)
    ).toBe(false)
    expect(
      shouldUseCustomWindowControls({ isFullscreen: true, nativeOverlayWidth: 0, windowButtonPosition: null }, true)
    ).toBe(false)
    expect(
      shouldUseCustomWindowControls({ isFullscreen: false, nativeOverlayWidth: 0, windowButtonPosition: null }, false)
    ).toBe(false)
  })
})
