...
export function TitlebarControls({ leftTools = [], tools = [], onOpenSettings }: TitlebarControlsProps) {
  ...
  const customWindowControls = shouldUseCustomWindowControls(connection, typeof window.hermesDesktop !== 'undefined')

  useEffect(() => {
    const root = document.documentElement

    if (customWindowControls) {
      root.style.setProperty('--custom-window-controls-width', `${CUSTOM_WINDOW_CONTROLS_WIDTH}px`)
    } else {
      root.style.removeProperty('--custom-window-controls-width')
    }
      ...
      */}
      ...
      <div
        ...
        className={cn(
          titlebarToolClusterClass,
          customWindowControls
            ? 'right-[calc(var(--titlebar-tools-right)+var(--custom-window-controls-width))] top-(--titlebar-controls-top)'
      ...
      {customWindowControls && (
        <CustomWindowControls
          ...
          labels={{ close: 'Close', maximize: 'Maximize', minimize: t.zones.minimize, restore: t.zones.restore }}
        />
      )}
    </>
  )
...
function CustomWindowControls({
  ...
  isMaximized: boolean
  labels: { close: string; maximize: string; minimize: string; restore: string }
}) {
  const run = (action: 'close' | 'minimize' | 'toggle-maximize') => {
    void window.hermesDesktop.controlWindow?.(action)
  }

  const controlClass =
    'grid h-(--titlebar-height) w-[46px] place-items-center bg-transparent text-(--ui-text-secondary) transition-colors duration-100 hover:bg-(--ui-control-hover-background) hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-(--theme-primary) [-webkit-app-region:no-drag]'

  return (
    <div
      aria-label="Window controls"
      className="fixed right-0 top-0 z-80 flex h-(--titlebar-height) items-stretch select-none [-webkit-app-region:no-drag]"
      data-custom-window-controls=""
      role="group"
    >
      <button aria-label={labels.minimize} className={controlClass} onClick={() => run('minimize')} type="button">
      ...
      <button
        ...
        type="button"
      >
        <Codicon name={isMaximized ? 'chrome-restore' : 'chrome-maximize'} size="0.8125rem" />
      </button>
      <button
        aria-label={labels.close}
        className={cn(controlClass, 'hover:bg-[#c42b1c] hover:text-white')}
        onClick={() => run('close')}
        type="button"
      >
        <Codicon name="chrome-close" size="0.9375rem" />
      </button>
    </div>
  )
}
