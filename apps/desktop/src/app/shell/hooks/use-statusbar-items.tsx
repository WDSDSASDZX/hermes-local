...
import { statusPerformanceLabels, statusPerformanceSummary } from '@/lib/status-performance'
...
export function useStatusbarItems({
  ...
  const performanceLabels = useMemo(() => statusPerformanceLabels(currentUsage), [currentUsage])
  const performanceSummary = useMemo(() => statusPerformanceSummary(currentUsage), [currentUsage])
  ...
  const coreRightStatusbarItems = useMemo<readonly StatusbarItem[]>(
    () => [
      ...
      {
        detail: performanceSummary.detail,
        hidden: !performanceSummary.label,
        icon: <Activity className="size-3" />,
        id: 'runtime-performance',
        label: performanceSummary.label,
        menuAlign: 'end',
        menuClassName: 'w-72 border-(--ui-stroke-secondary) p-0',
        menuContent: (
          <div className="p-3">
            <div className="text-xs font-semibold text-(--ui-text-primary)">{copy.performanceTitle}</div>
            <div className="mt-2 space-y-1 font-mono text-[0.6875rem] text-(--ui-text-secondary)">
              {performanceLabels.map(label => (
                <div className="flex min-h-5 items-center" key={label}>
                  {label}
                </div>
              ))}
            </div>
            <div className="mt-2 border-t border-(--ui-stroke-tertiary) pt-2 text-[0.625rem] text-(--ui-text-quaternary)">
              {copy.performanceScope}
            </div>
          </div>
        ),
        toggleLabel: copy.togglePerformance,
        variant: 'menu'
      },
      {
