...
export const CUSTOM_WINDOW_CONTROLS_WIDTH = 138
...
export function shouldUseCustomWindowControls(
  connection: Pick<HermesConnection, 'isFullscreen' | 'nativeOverlayWidth' | 'windowButtonPosition'> | null | undefined,
  desktopBridgeAvailable: boolean
): boolean {
  return Boolean(
    desktopBridgeAvailable &&
    connection &&
    !connection.isFullscreen &&
    connection.windowButtonPosition === null &&
    connection.nativeOverlayWidth <= 0
  )
}

export const titlebarHeaderBaseClass =
  'pointer-events-none relative z-3 flex h-(--titlebar-height) w-full min-w-0 shrink-0 items-center justify-start gap-3 overflow-hidden border-b border-(--ui-stroke-tertiary) bg-(--ui-chat-surface-background) px-[max(0.75rem,var(--titlebar-content-inset,0rem))] pr-[calc(var(--titlebar-tools-right,0.75rem)+var(--titlebar-tools-width,0px)+var(--custom-window-controls-width,0px)+0.75rem)]'
