...
function formFromEndpoint(endpoint: CustomEndpoint): EndpointForm {
  return {
    anthropicAuth: endpoint.anthropic_auth,
    apiKey: '',
...
function toPayload(form: EndpointForm, models?: string[]): CustomEndpointUpdate {
  ...
  return {
    ...
    anthropic_auth: form.anthropicAuth,
...
export function CustomEndpointsSettings({ onConfigSaved, onMainModelChanged }: CustomEndpointsSettingsProps) {
  ...
  async function handleValidate() {
    try {
      ...
      const latency = response.latency_ms === null ? '' : ' in ' + response.latency_ms + ' ms'
      ...
      if (response.ok) {
        ...
        notify({
          ...
          message: response.models.length
            ? 'Endpoint is reachable. Found ' + response.models.length + ' models' + latency + '.'
            : (response.message || 'Endpoint is reachable.') + latency
        })
      } else {
        notify({
          kind: response.reachable ? 'warning' : 'error',
          message: (response.message || 'Endpoint validation failed.') + latency
        })
      }
    } catch (err) {
  ...
  return (
    <SettingsContent>
      <div className="space-y-6">
        <section>
          <SectionHeading icon={Globe} meta={`${endpoints.length}`} title="Relay Providers" />
          <p className="mb-3 text-xs leading-5 text-muted-foreground">
            Configure Claude/Anthropic-native and OpenAI-compatible relays here. Keys stay in the local secret store.
          </p>
        ...
        <section>
          <SectionHeading icon={Plus} title={form.id ? 'Edit Relay' : 'Add Relay'} />
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1.5 text-xs text-muted-foreground">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="codex_responses">OpenAI Responses / Codex</SelectItem>
                    <SelectItem value="bedrock_converse">AWS Bedrock Converse</SelectItem>
                  </SelectContent>
                </Select>
              </label>
              {form.apiMode === 'anthropic_messages' && (
                <label className="grid gap-1.5 text-xs text-muted-foreground">
                  ...
                  <Select
                    onValueChange={value =>
                      setForm(current => ({ ...current, anthropicAuth: value as AnthropicAuthMode }))
                    }
