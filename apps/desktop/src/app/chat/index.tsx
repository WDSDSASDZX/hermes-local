...
function ChatHeader({
  ...
  // Secondary windows stay compact. The primary window keeps a header even
  // for a brand-new draft so the workspace has a stable, readable hierarchy.
  if (isAuxiliaryWindow()) {
    return null
  }

  const hasSessionActions = Boolean(selectedSessionId || activeSessionId)
    ...
    <header className={cn(titlebarHeaderBaseClass, isRoutedSessionView && titlebarHeaderShadowClass)}>
      ...
      >
        ...
        {hasSessionActions ? (
          ...
          <span className="block truncate text-[0.9375rem] font-semibold tracking-[-0.015em] text-foreground">
            {title}
          </span>
        )}
      </div>
