...
export function TitleMenuTrigger({
  ...
  return (
    <Button
      className={cn(
        'pointer-events-auto flex h-7 min-w-0 max-w-full gap-1.5 overflow-hidden border border-transparent bg-transparent px-2.5 py-0 text-(--ui-text-secondary) hover:border-(--ui-stroke-tertiary) hover:bg-(--ui-control-hover-background) hover:text-foreground data-[state=open]:border-(--ui-stroke-tertiary) data-[state=open]:bg-(--ui-control-active-background) [-webkit-app-region:no-drag]',
        className
      )}
      type="button"
      variant="ghost"
      {...props}
    >
      <span className="min-w-0 flex-1 truncate text-[0.875rem] font-semibold leading-none">{children}</span>
