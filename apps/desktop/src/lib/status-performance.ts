...
const compactTokens = (value: number): string => {
  ...
  if (safe < 1_000_000) return `${Number((safe / 1_000).toFixed(safe < 10_000 ? 1 : 0))}k`
...
export function statusPerformanceLabels(usage: UsageStats): string[] {
  ...
  if (lastTurn > 0) labels.push(`last response ${formatMetricSeconds(lastTurn)}`)
  if (turns || toolCalls) {
    labels.push([turns ? `${turns} turns` : '', toolCalls ? `${toolCalls} tools` : ''].filter(Boolean).join(' · '))
  } else if (calls) labels.push(`${calls} LLM calls`)
  ...
  if ((cacheRead || cacheWrite) && cacheBase > 0) labels.push(`cache ${Math.round((cacheRead / cacheBase) * 100)}%`)
...
export function statusPerformanceSummary(usage: UsageStats): { detail?: string; label?: string } {
  ...
  const speed = [
    ...
    .filter(Boolean)
    .join(' · ')
  if (lastTurn > 0) return { detail: speed || undefined, label: `✓ ${formatMetricSeconds(lastTurn)}` }
