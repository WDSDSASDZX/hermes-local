...
class TestBuildAnthropicClient:
    ...
    def test_agentrouter_endpoint_gets_required_cli_user_agent(self):
        with patch("agent.anthropic_adapter._anthropic_sdk") as mock_sdk:
            build_anthropic_client(
                "agentrouter-secret",
            ...
            kwargs = mock_sdk.Anthropic.call_args[1]
            headers = kwargs["default_headers"]
            assert headers["User-Agent"] == "claude-cli/1.0.0 (external, cli)"
            assert "anthropic-beta" in headers

        with patch("agent.anthropic_adapter._anthropic_sdk") as mock_sdk:
            build_anthropic_client(
                "lookalike-secret",
                base_url="https://agentrouter.org.attacker.test/v1",
            )
            headers = mock_sdk.Anthropic.call_args[1].get("default_headers", {})
            assert "User-Agent" not in headers

    def test_configured_headers_override_agentrouter_defaults(self):
        configured = {
            "User-Agent": "relay-approved-client/2.0",
            "X-Relay-Route": "primary",
        }
        with (
            patch(
        ...
        assert headers["User-Agent"] == "relay-approved-client/2.0"
        assert headers["X-Relay-Route"] == "primary"
        assert "anthropic-beta" in headers

    def test_agentrouter_user_agent_survives_tool_result_followup(self):
        received = []
        ...
        try:
            with patch(
                "agent.anthropic_adapter._is_agentrouter_endpoint",
                return_value=True,
            ):
                client = build_anthropic_client("relay-secret", base_url)
                try:
            ...
            assert [request["user_agent"] for request in received] == [
                "claude-cli/1.0.0 (external, cli)",
                "claude-cli/1.0.0 (external, cli)",
            ]
            assert received[1]["body"]["messages"][-1]["content"][0]["type"] == "tool_result"
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=2)

    def test_configured_relay_can_force_bearer_auth(self):
        relay = {
            "name": "Claude Relay",
        ...
        with (
            patch("hermes_cli.config.get_compatible_custom_providers", return_value=[relay]),
            patch("agent.anthropic_adapter._anthropic_sdk") as mock_sdk,
        ):
            build_anthropic_client("relay-test-secret", "https://relay.example.test")
            kwargs = mock_sdk.Anthropic.call_args[1]
            assert kwargs["auth_token"] == "relay-test-secret"
            assert "api_key" not in kwargs

    def test_configured_bearer_relay_sends_a_real_messages_request(self):
        received = {}
        ...
        class RelayHandler(BaseHTTPRequestHandler):
            def do_POST(self):
                ...
                received["x_api_key"] = self.headers.get("x-api-key")
        ...
        try:
            with patch(
                ...
                client = build_anthropic_client("local-relay-secret", base_url)
            ...
            assert received["authorization"] == "Bearer local-relay-secret"
            assert received["x_api_key"] is None
            assert received["body"]["model"] == "claude-relay-test"
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=2)
