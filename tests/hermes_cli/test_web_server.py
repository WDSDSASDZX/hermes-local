...
class TestWebServerEndpoints:
    ...
    def test_openai_relay_root_is_normalized_on_save_and_activate(self):
        """GUI root URLs must persist as OpenAI-compatible /v1 base URLs."""
        from hermes_cli.config import load_config, save_config
        ...
        assert response.status_code == 200
        cfg = load_config()
        assert cfg["providers"]["openai-relay"]["base_url"] == "https://relay.example.test/v1"
        assert cfg["model"]["base_url"] == "https://relay.example.test/v1"

        cfg["providers"]["legacy-relay"] = {
            "name": "Legacy Relay",
        ...
        activated = self.client.post(
            "/api/providers/custom-endpoints/legacy-relay/activate", json={}
        )
        assert activated.status_code == 200
        cfg = load_config()
        assert cfg["providers"]["legacy-relay"]["base_url"] == "https://legacy.example.test/v1"
        assert cfg["model"]["base_url"] == "https://legacy.example.test/v1"


    def test_anthropic_relay_persists_protocol_auth_and_active_runtime(self):
        """A Claude relay profile must survive save and drive the real runtime mode."""
        from hermes_cli.config import load_config
        ...
        assert response.status_code == 200
        endpoint = next(e for e in response.json()["endpoints"] if e["id"] == "claude-relay")
        assert endpoint["api_mode"] == "anthropic_messages"
        assert endpoint["anthropic_auth"] == "bearer"
        assert endpoint["has_api_key"] is True

        cfg = load_config()
        assert cfg["providers"]["claude-relay"]["api_mode"] == "anthropic_messages"
        assert cfg["providers"]["claude-relay"]["anthropic_auth"] == "bearer"
        assert cfg["model"]["provider"] == "claude-relay"
        assert cfg["model"]["api_mode"] == "anthropic_messages"
        assert "api_key" not in cfg["providers"]["claude-relay"]
        ...
        client_kwargs = mock_sdk.Anthropic.call_args.kwargs
        assert client_kwargs["auth_token"] == "relay-test-secret"
...
class TestValidateProviderCredential:
    ...
    def test_named_custom_endpoint_probe_is_async(self, monkeypatch):
        ...
        assert captured == {
            ...
            "headers": {
                ...
                "User-Agent": "Hermes-Local/1.0",
                "Authorization": "Bearer local-secret",
            },
        }

    def test_anthropic_relay_probe_uses_bearer_v1_models_and_reports_latency(self, monkeypatch):
        from types import SimpleNamespace
        import hermes_cli.web_server as web_server
        ...
        class _Resp:
            ...
            def json(self):
                return {"data": [{"id": "claude-opus-test"}]}
        ...
        monkeypatch.setattr("httpx.AsyncClient", _Client)
        monkeypatch.setattr(web_server, "time", SimpleNamespace(monotonic=lambda: next(ticks)))
        ...
        assert payload["models"] == ["claude-opus-test"]
        assert captured == {
    ...
    def test_custom_endpoint_probe_reports_cloudflare_block_before_auth(self, monkeypatch):
        class _Resp:
            status_code = 403
            is_success = False
            headers = {
    ...
    def test_anthropic_probe_falls_back_to_messages_when_models_rejects(self, monkeypatch):
        from types import SimpleNamespace
        import hermes_cli.web_server as web_server
        ...
        def _build_probe_client(api_key, base_url, **kwargs):
            captured["sdk"] = {
                "api_key": api_key,
                "base_url": base_url,
                **kwargs,
        ...
        assert captured == {
            "get_url": "https://relay.example.test/v1/models",
            "sdk": {
                "api_key": "relay-test-secret",
                "base_url": "https://relay.example.test",
                "timeout": 30.0,
            ...
            "messages": {
                "model": "claude-opus-test",
                "max_tokens": 1,
                "messages": [{"role": "user", "content": "Reply with one character: K"}],
            },
            "closed": True,
        }
