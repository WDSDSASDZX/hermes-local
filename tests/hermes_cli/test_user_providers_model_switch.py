...
def test_current_model_is_not_injected_into_another_configured_relay(monkeypatch):
    """A model owned by another relay must keep that relay's protocol and key."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("hermes_cli.providers.HERMES_OVERLAYS", {})
    monkeypatch.setenv("CLAUDE_RELAY_KEY", "sk-test-claude")
    monkeypatch.setenv("GPT_RELAY_KEY", "sk-test-gpt")
    ...
    providers = list_authenticated_providers(
        current_provider="claude-relay",
        current_model="gpt-test",
        user_providers=user_providers,
        custom_providers=[],
    )

    claude_row = next(row for row in providers if row["slug"] == "claude-relay")
    gpt_row = next(row for row in providers if row["slug"] == "gpt-relay")
    assert claude_row["models"] == ["claude-opus-test"]
