...
_CUSTOM_ENDPOINT_API_MODES = {
    "chat_completions", "anthropic_messages", "codex_responses", "bedrock_converse"
}
_CUSTOM_ENDPOINT_ANTHROPIC_AUTH = {"auto", "x_api_key", "bearer"}
_CUSTOM_ENDPOINT_PROBE_USER_AGENT = "Hermes-Local/1.0"


def _custom_endpoint_api_mode(raw: Any) -> str:
    from hermes_cli.config import _canonical_api_mode

    normalized = _canonical_api_mode(str(raw or "chat_completions")).strip().lower()
    return normalized if normalized in _CUSTOM_ENDPOINT_API_MODES else "chat_completions"


def _custom_endpoint_base_url(raw: Any, api_mode: str) -> str:
    """Normalize relay roots to the base URL expected by the selected SDK."""
    base_url = str(raw or "").strip().rstrip("/")
    if not base_url or api_mode not in {"chat_completions", "codex_responses"}:
        return base_url
...
def _custom_endpoint_response(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(providers, dict):
        for provider_id, raw_entry in providers.items():
            ...
            endpoints.append({
                ...
                "api_mode": _custom_endpoint_api_mode(raw_entry.get("api_mode") or raw_entry.get("transport")),
                "anthropic_auth": str(raw_entry.get("anthropic_auth") or "auto"),
    ...
    if current_provider.lower() == "custom" and current_base_url and not any(e["id"] == "custom" for e in endpoints):
        ...
        endpoints.insert(0, {
            ...
            "api_mode": _custom_endpoint_api_mode(model_cfg.get("api_mode")),
            "anthropic_auth": "auto",
            "has_api_key": has_api_key,
            "api_key_preview": api_key_preview,
            "is_current": True,
...
def _write_custom_endpoint(cfg: Dict[str, Any], body: CustomEndpointUpdate) -> Tuple[str, Dict[str, Any]]:
    ...
    model = (body.model or "").strip()
    from hermes_cli.config import _canonical_api_mode
    api_mode = _canonical_api_mode(body.api_mode or "chat_completions").strip().lower()
    base_url = _custom_endpoint_base_url(body.base_url, api_mode)
    anthropic_auth = (body.anthropic_auth or "auto").strip().lower()

    if api_mode not in _CUSTOM_ENDPOINT_API_MODES:
        raise HTTPException(status_code=400, detail="unsupported api_mode")
    if anthropic_auth not in _CUSTOM_ENDPOINT_ANTHROPIC_AUTH:
        raise HTTPException(status_code=400, detail="unsupported anthropic_auth")
...
def activate_custom_endpoint(endpoint_id: str, profile: Optional[str] = None):
    ...
    try:
        with _config_profile_scope(profile):
            ...
            api_mode = _custom_endpoint_api_mode(
                entry.get("api_mode") or entry.get("transport")
            )
            base_url = _custom_endpoint_base_url(entry.get("base_url"), api_mode)
...
async def validate_custom_endpoint(body: CustomEndpointUpdate, profile: Optional[str] = None):
    """Probe a relay with protocol-aware authentication and report latency."""
    import httpx

    api_mode = _custom_endpoint_api_mode(body.api_mode)
    base_url = _custom_endpoint_base_url(body.base_url, api_mode)
    if not base_url:
        return {
            "ok": False, "reachable": True, "message": "Enter an endpoint URL first.",
    ...
    if not api_key and body.id:
        with _config_profile_scope(profile):
            cfg = load_config()
            providers = cfg.get("providers")
            entry = providers.get(_custom_endpoint_id(body.id)) if isinstance(providers, dict) else None
    ...
    if api_mode == "anthropic_messages" and not base_url.lower().endswith("/v1"):
        url = base_url + "/v1/models"
    else:
        url = base_url + "/models"

    # Identify the request explicitly instead of inheriting httpx's default
    # ``python-httpx/*`` user agent. Some API-facing Cloudflare rules reject
    # generic HTTP-library user agents before the request reaches API auth.
    headers = {
        "Accept": "application/json",
        "User-Agent": _CUSTOM_ENDPOINT_PROBE_USER_AGENT,
    }
    if api_mode == "anthropic_messages":
        headers["anthropic-version"] = "2023-06-01"
        if api_key:
            use_bearer = anthropic_auth == "bearer"
            if anthropic_auth == "auto":
                from agent.anthropic_adapter import _requires_bearer_auth

                use_bearer = _requires_bearer_auth(base_url)
            if use_bearer:
    ...
    try:
        ...
        return {
            "ok": False, "reachable": False, "message": f"Could not reach {url}.",
            "models": [], "latency_ms": latency_ms,
        }

    if api_mode == "anthropic_messages" and resp.status_code in (401, 403):
        # Some Anthropic relays authenticate /v1/messages normally but block
        # /v1/models entirely. Treating that policy response as a bad key
        # produces a false negative (agentrouter.org is one such relay).
        # Fall back to the smallest possible Messages request so Test verifies
        # the exact path Hermes actually uses. This can consume one output token.
        from agent.anthropic_adapter import build_anthropic_client

        def _probe_messages():
            probe_client = build_anthropic_client(
                api_key,
                base_url,
                timeout=30.0,
                anthropic_auth=anthropic_auth,
            ...
            try:
                return probe_client.messages.create(
                    model=(body.model or "").strip(),
                    max_tokens=1,
                    messages=[
                        {"role": "user", "content": "Reply with one character: K"}
                    ],
                )
            finally:
        ...
        try:
            await asyncio.to_thread(_probe_messages)
        except Exception as exc:
            latency_ms = round((time.monotonic() - started) * 1000)
            ...
            if type(exc).__name__ == "APITimeoutError":
                return {
                    "ok": False, "reachable": True,
                    "message": "Messages authentication probe timed out upstream.",
                    "models": [], "latency_ms": latency_ms,
                }
            return {
                "ok": False, "reachable": True,
                "message": "Model discovery is blocked and the Messages probe failed.",
                "models": [], "latency_ms": latency_ms,
            }
        ...
        return {
            ...
            "message": "Messages authenticated; model discovery is unavailable.",
            "models": [model] if model else [], "latency_ms": latency_ms,
        }
    ...
    response_text = str(getattr(resp, "text", "")).lower()
    if resp.status_code == 403 and (
        "cloudflare" in server
        or "cloudflare" in response_text
        or (
            "text/html" in content_type
            and "sorry, you have been blocked" in response_text
        )
    ):
        return {
            "ok": False,
            "reachable": True,
            "message": (
                "Cloudflare blocked the Hermes probe before API key validation."
            ),
            "models": [],
    ...
    if resp.status_code in (401, 403):
        return {
            "ok": False, "reachable": True, "message": "The endpoint rejected the API key.",
            "models": [], "latency_ms": latency_ms,
        }
    if api_mode == "anthropic_messages" and resp.status_code in (404, 405):
        return {
            "ok": True, "reachable": True,
            "message": "Endpoint is reachable; this relay does not expose model discovery.",
            "models": [], "latency_ms": latency_ms,
        }
    if not resp.is_success:
        return {
    ...
    return {
        ...
        "models": _parse_model_ids(resp), "latency_ms": latency_ms,
    }
