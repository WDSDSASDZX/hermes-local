...
def _desktop_linux_wsl_uses_userns_sandbox() -> bool:
    """Use Chromium user namespaces on WSL without an interactive sudo prompt."""
    if sys.platform != "linux":
        return False
    try:
...
def cmd_gui(args: argparse.Namespace):
    ...
    if _desktop_linux_wsl_uses_userns_sandbox():
        # WSL supports Chromium user namespaces, but an unpacked Electron
        # helper is user-owned after every local rebuild. Disable only the SUID
        # helper; the user-namespace sandbox remains enabled.
