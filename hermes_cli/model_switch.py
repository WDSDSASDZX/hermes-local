...
def list_authenticated_providers(
    ...
    *,
    ...
    if current_model:
        # If another configured provider explicitly owns this model, do not
        # inject it into the current row. Relays on the same host can use
        # different wire protocols and credentials; making a GPT model appear
        # under an Anthropic-Messages row lets the picker create an invalid
        # model/provider pairing that later fails as a malformed stream.
        _declared_by_another_row = any(
            not _row.get("is_current")
            and current_model in (_row.get("models") or [])
            for _row in results
        )
        if not _declared_by_another_row:
            for _row in results:
